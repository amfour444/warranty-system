...
export default function AddOrder() {
  ...
  const [storeOptions, setStoreOptions] = useState<any[]>([]); // เก็บรายชื่อร้านค้า

  // โหลดร้านค้าจาก Database
  useEffect(() => {
  ...
  const startScanner = () => {
    setIsScanning(true);
    setTimeout(() => {
      const scanner = new Html5QrcodeScanner("reader-add", { fps: 10, qrbox: {width: 250, height: 250}, videoConstraints: { facingMode: "environment" } }, false);
      scanner.render((txt) => {
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4 relative overflow-hidden">
            ...
            <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 flex justify-between items-center relative z-10"><span className="text-sm font-bold text-emerald-700">วันหมดอายุประกัน:</span><span className="text-lg font-black text-emerald-600">{format(parseISO(formData.expiry_date), 'dd/MM/yyyy')}</span></div>
          </div>

          {/* Customer Info (พร้อม Dropdown ร้านค้า) */}
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
          ...
          <button type="submit" disabled={loading} className="w-full bg-slate-900 text-white p-4 rounded-2xl font-black text-lg shadow-xl shadow-slate-300 hover:bg-slate-800 transition-all active:scale-[0.98] flex items-center justify-center gap-2">{loading ? <Loader2 className="animate-spin"/> : <Save size={20}/>} {loading ? 'Saving...' : 'Save Order'}</button>
        </form>

        {isScanning && <div className="fixed inset-0 bg-slate-900/95 z-50 flex flex-col items-center justify-center p-6 backdrop-blur-sm"><div className="bg-white p-6 rounded-3xl w-full max-w-xs shadow-2xl"><h3 className="font-black text-slate-800 mb-4 text-center">Scan Serial Number</h3><div id="reader-add" className="rounded-2xl overflow-hidden border-4 border-slate-50 aspect-square"></div><button onClick={()=>setIsScanning(false)} className="w-full mt-6 p-3 bg-slate-100 text-slate-500 rounded-xl font-bold flex justify-center gap-2 items-center"><X size={18}/> Close Scanner</button></div></div>}
      </div>
    </div>
  );
}